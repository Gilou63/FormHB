...
//teeeeeeest
    ...
    public String sayHello() {
        ...
        String formulaire = "<form action='/' method='POST'>"+
                ...
                "-> Date de debut : <input type='text' name='dateDebut' /> "+
                "<input type='submit' />"+
                "</form>";

        for (Task task : tasks) {
            taskHTML += "<div> <a href='/view/" + i + "' title='voir " + task.getTitle() + " ?'>" + task.getTitle() +
                    "</a> -" + task.getDateFin() + "- <a href='/delete/" + i + "' title='Supprimer " + task.getTitle() +
                    " ?'>x</a></div><br>";
            i++;
        }
...
//tessssssssssssssssst
    ...
    public String delete(@PathParam("id") int id) {
        ...
        return "<b>Todo :</b> " + taskToDisplay.getTitle() + " <br>date de fin : " +
                taskToDisplay.getDateFin() + "<br><br><a href='/'>Retour</a>";
    }
